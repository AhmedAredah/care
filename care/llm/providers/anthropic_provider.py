"""Anthropic Claude cloud provider (Phase 12).

DISABLED BY DEFAULT. Network required. Sends data to Anthropic.
Refused in offline mode. SDK (``anthropic``) imported inside
``load()``.
"""
...
class AnthropicProvider(LLMProvider):
    ...
    def load(self, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise ConfigError(
                f"{self.provider_name}: anthropic SDK is not installed."
            ) from exc
    ...
    def healthcheck(self) -> ProviderHealth:
        return ProviderHealth(
            ...
            detail="anthropic" if self._loaded else "not loaded",
        )
    ...
    def get_model_manifest(self) -> dict[str, Any]:
        ...
        return {
            ...
            "vendor": self.vendor,
            "model_name": safe_config.get("model", "claude-haiku-4-5"),
